package global.sesoc.personal.util;
...
public class FileService {
	...
	public static String saveFile(MultipartFile upload, String uploadPath) {
		...
		//HDD에 저장할 파일명. 같은 이름의 파일이 있는 경우의 처리 사용할일 낮음
		serverFile = new File(uploadPath + "/" + filename);
		/*
		 * while (true) { serverFile = new File(uploadPath + "/" + filename); //같은
		 * 이름의파일이 없으면 나감. if ( !serverFile.isFile()) break; //같은 이름의 파일이 있으면 이름 뒤에 long
		 * 타입의시간정보를 덧붙임. savedFilename = filename + new Date().getTime(); }
		 */
